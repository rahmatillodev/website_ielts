                  ...
                  {title !== "Writing" && (
                    <>
                      <div className='p-3 bg-gray-50 rounded-lg'>
                        ...
                        <p className='font-semibold text-gray-700'>{item.total_questions ?? 'N/A'}</p>
                      </div>
                      <div className='p-3 bg-green-50 rounded-lg'>
                        <p className='text-xs text-green-600 uppercase font-bold mb-1'>Correct</p>
                        <div className='flex items-center gap-1 text-green-700'>
                          <CheckCircle2 size={14} />
                          <span className='font-semibold'>{item.correct_answers ?? 'N/A'}</span>
                        </div>
                      </div>
                    </>
                  )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-gray-50 rounded-xl">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${
                      client.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
                    }`}>
                      {client.status}
                    </span>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-xl">
                    <span className="text-xs font-bold text-gray-400 uppercase block mb-1">User Name</span>
                    <span className="font-mono text-sm text-gray-600">{client.full_name}</span>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-xl">
                    <span className="text-xs font-bold text-gray-400 uppercase block mb-1">User Email</span>
                    <span className="font-mono text-sm text-gray-600">{client.email}</span>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-xl">
                    <span className="text-xs font-bold text-gray-400 uppercase block mb-1">User Phone</span>
                    <span className="font-mono text-sm text-gray-600">{client.phone_number}</span>
                  </div>
                </div>
              </AccordionContent>
            </AccordionItem>
