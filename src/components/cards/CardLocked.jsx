...
const CardLocked = ({
  ...
  // Every card in this file is locked, so the edge is the warning accent unless
  // the test has already been completed, which the success accent outranks.
  const borderState = isCompleted ? 'completed' : 'locked';
  const iconState = isCompleted ? 'completed' : 'locked';
  const tier = is_premium ? 'premium' : 'free';

  const containerClass = isGridView
    ? `bg-white border ${CARD_BORDER[borderState]} rounded-2xl p-4 shadow-lg hover:shadow-2xl transition-all flex flex-col relative h-full`
    : `bg-white border border-l-4 ${CARD_BORDER_LEFT[borderState]} rounded-xl md:rounded-[24px] p-3 md:p-4 shadow-lg hover:shadow-2xl flex items-center gap-3 md:gap-4 justify-between mb-4`;
  ...
  if (isGridView) {
    ...
    return (
      ...
      >
        {/* Premium/Free Badge */}
        <div className={`${isCompleted ? 'absolute top-3 right-20 md:right-24 z-10' : 'absolute top-3 right-3 z-10'}`}>
        ...
        {isCompleted && (
          <div className="absolute top-3 right-3 z-10">
            <div className="bg-white border border-gray-200 rounded-full px-2 py-1 flex items-center gap-1 shadow-sm">
              ...
              <span className={`text-sm font-black ${CARD_SCORE}`}>{formatScore(score, '0.0')}</span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col flex-1">
          {/* Icon */}
          <div className={`size-12 md:size-14 mb-4 rounded-xl ${CARD_ICON[iconState]} flex items-center justify-center shrink-0`}>
            {isCompleted ? (
              <MdCheckCircle className="text-2xl md:text-2xl" />
            ) : (
              <MdLock className="text-2xl md:text-2xl" />
            )}
          </div>
          ...
          <UpgradeModal>
            <button className={`mt-4 py-2.5 font-black rounded-lg transition-all w-full flex items-center justify-center gap-2 text-xs shadow-md ${CARD_CTA}`}>
    ...
    return (
      ...
      >
        {/* Icon */}
        <div className={`size-10 md:size-14 rounded-xl md:rounded-2xl ${CARD_ICON[iconState]} flex items-center justify-center shrink-0`}>
          {isCompleted ? (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2 mb-2 flex-wrap">
            ...
            {!isCompleted && (
              <span className={`ml-2 md:ml-4 px-2.5 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-black uppercase rounded-lg md:rounded-xl tracking-wider flex items-center gap-1.5 shrink-0 self-start ${TIER_BADGE[tier]}`}>
                {is_premium && <FaCrown className="text-xs md:text-sm" />} {cardStatus}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-3 md:gap-6 shrink-0">
          {isCompleted ? (
            ...
            <UpgradeModal>
              ...
              <button className={`py-2 md:py-3 flex items-center gap-2 px-4 md:px-8 font-black rounded-lg md:rounded-xl transition-all shrink-0 text-xs md:text-sm shadow-md ${CARD_CTA}`}>
