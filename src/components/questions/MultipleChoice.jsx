        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            <tr className={`border-t border-gray-200 transition-colors ${
              showWrong ? 'bg-danger-50' : showCorrect ? 'bg-success-50' : 'bg-white'
            }`}>
              {/* Question Column */}
              <td className={`px-4 py-3 text-gray-900 relative group ${showWrong ? 'bg-danger-50' : showCorrect ? 'bg-success-50' : ''}`}>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {showWrong && correctAnswer && showCorrectAnswers && (
                    <span className="text-xs text-success-600 font-medium ml-2">Correct: {correctAnswer}</span>
                  )}
                </div>
      ...
      {sortedOptions.map((option, index) => {
        ...
        return (
          <label
            ...
            className={`flex gap-3 items-center p-2 rounded-md transition-all ${
              ...
              isSelected && showCorrect
                ? "bg-success-100 border-2 border-success-500 text-success-900"
          ...
          >
            ...
            {isSelected && showCorrect && (
              <span className="text-xs text-success-700 font-medium">Correct</span>
            )}
            {isSelected && showWrong && (
            ...
            {isCorrectOption && isReviewMode && !isSelected && correctAnswer && showCorrectAnswers && (
              <span className="text-xs text-success-700 font-medium">Correct Answer</span>
            )}
          </label>
        );
      })}
