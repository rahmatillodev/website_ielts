...
> **⚠️ Important for Users Website**: If you're building the **users website** (student-facing application), **DO NOT use the `getTestById` function** from the admin store. Instead, see **[USER_WEBSITE_TEST_FETCHING.md](../USER_WEBSITE_TEST_FETCHING.md)** for the correct approach to fetch test data with proper security, RLS policies, and data filtering.
...
> **⚠️ For Users Website**: The query below is for **admin applications only**. For the users website, see **[USER_WEBSITE_TEST_FETCHING.md](../USER_WEBSITE_TEST_FETCHING.md)** which explains why this approach won't work and provides the correct implementation.

### Using Supabase Query (Admin Only)
